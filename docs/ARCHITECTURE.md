...
- **Sales** (`/sales`, `sales` module): `POST` prices the lines from the catalog (`priceSale`: the product name is copied, `unitPrice` defaults to the product price, repeated products at the same price are merged, lines keep their `position`), numbers the sale per business (`number`, retried on a concurrent clash), takes counted products (`trackStock`) out of stock with one `StockMovement` (`sale`, negative quantity) each and updates `Product.stock` in the same transaction. **Stock may go negative: a sale is never blocked**; the response lists `lowStock` (products at or below `minStock`, or 0). `paymentType: cash` needs `method`; `credit` needs `customerId` and `dueDate` and creates the receivable (`summarizeItems` as description, `saleId` link), so it shows in "Me deben" and follows the usual reminders and payments. Receipts are recorded (`docType`: none / sale_note / receipt / invoice, `docNumber`), not issued. `POST /sales/:id/void` puts the stock back (`sale_void` movements) and removes the receivable; it fails with `422 SALE_HAS_PAYMENTS` when the credit sale already has payments. Products that appear in sales cannot be deleted (`409 PRODUCT_IN_USE`, archive them).
