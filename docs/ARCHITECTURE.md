...
- **Cross-filters:** optional `method`, `customerId` and `weekday` (ISO, 1 = Monday), validated by `analyticsFiltersSchema`. Payment data (collected, payments, averages, series) honours all of them; receivables (issued, due, collection rate) and reminders only `customerId`; new customers and the snapshot none. Each breakdown ignores its own dimension so the dashboard can keep showing the other options: `byMethod` skips `method`, `byWeekday` skips `weekday` (it is computed from the same daily rows before the weekday filter), `topPayers` skips `customerId`. The response echoes them in `filters` (with the customer's name for the chip).
...
Solvia is sold as **modules per business** (`Tenant.modules`, enum `TenantModule`, at least one, any combination): `collections` (Cobranza: receivables, payments, WhatsApp reminders, statements, dashboard and the collections report), `sales` (Ventas: sales with their items, cash, or on credit when Cobranza is also enabled) and `inventory` (Inventario: purchases, suppliers and stock). A business can have only Ventas or only Inventario. Two shared capabilities are not modules of their own: the **product catalog** comes with Ventas or Inventario (`hasModule(modules, 'catalog')`) and **customers** with Cobranza or Ventas (`hasModule(modules, 'customers')`). Businesses without Cobranza are skipped by the reminder and monthly report jobs. The backoffice enables them per business (`PATCH /admin/tenants/:id { modules }`, the full list); `GET /auth/me` returns them in `tenant.modules` so the frontends show only the menus of enabled modules. Routers of a module run behind `requireModule(...)`, which answers `403 MODULE_NOT_ENABLED`.

- **Products** (`/products`): `kind` product or service (a service is never counted: no stock control, alert level or sack size), name, a code (barcode or internal, unique per business: `409 PRODUCT_CODE_TAKEN`; without one, or when cleared, the product gets the next internal code "2" + 7 digits, the GS1 in-store range, which the app prints as its QR; `productCode.ts`), an optional picture (`PUT /products/:id/image`, multipart field `image`, JPEG/PNG/WEBP, `400 INVALID_IMAGE_TYPE`; `DELETE` removes it; served under `/files/products/`), unit (`ProductUnit`), sale price, optional cost, `trackStock` (false for services or items not counted) optional `minStock` (3 decimals) and optional `packSize`: how much of its unit the sack/box it is bought in holds (e.g. a 10 kg sack of rice sold by the kilo; the app lets purchases be entered in sacks and converts them to the unit, so stock, costs and sales stay in kilos, and quantities take up to 3 decimals: 0.3 kg = 300 g). `active: false` archives a product (hidden from pickers, kept for history); only admins delete. Partial updates never reset omitted fields (the update schema has no defaults).
- **Sales** (`/sales`, `sales` module): `POST` prices the lines from the catalog (`priceSale`: the product name is copied, `unitPrice` defaults to the product price, repeated products at the same price are merged, lines keep their `position`; lines without `productId` are free lines — a service or something not in the catalog, with `description` and `unitPrice` — that never move stock; `discount` comes off the sum of the lines, `400 DISCOUNT_TOO_HIGH` when it is more; optional `notes`), numbers the sale per business (`number`, retried on a concurrent clash), takes counted products (`trackStock`) out of stock with one `StockMovement` (`sale`, negative quantity) each and updates `Product.stock` in the same transaction. **Stock may go negative: a sale is never blocked**; the response lists `lowStock` (products at or below `minStock`, or 0). `paymentType: cash` needs `method`; `credit` needs `customerId` and `dueDate` and creates the receivable (`summarizeItems` as description, `saleId` link; an optional `downPayment` with `downPaymentMethod` is its first payment and must be less than the total, `400 DOWN_PAYMENT_TOO_HIGH`), so it shows in "Me deben" and follows the usual reminders and payments. Receipts are recorded (`docType`: none / sale_note / receipt / invoice, `docNumber`), not issued. `POST /sales/:id/void` puts the stock back (`sale_void` movements) and removes the receivable; it fails with `422 SALE_HAS_PAYMENTS` when the credit sale already has payments. Products that appear in sales cannot be deleted (`409 PRODUCT_IN_USE`, archive them).
- **Stock traceability**: every `StockMovement` keeps `balanceAfter` (the stock it left) and `shortage` (the part that left without stock to cover it: `shortageOf(stockBefore, quantity)`); the sale line (`SaleItem.shortage`) and the sale (`hasShortage`, filter `GET /sales?shortage=true`) are marked too. `GET /products/:id/movements` is the product's kardex (newest first, with the sale number), so a difference between a physical count and the system can be traced to the sales made without stock.
- **Picker searches** (`GET /products/lookup`, `GET /customers/lookup`, `?search=&limit=` up to 20): light raw-SQL queries with explicit tenant filter, no count and no heavy includes (the customer balance is summed in SQL instead of loading receivables). Products: exact code first (a scanned barcode), then names starting with the text, then containing it; customers: without text the ones who owe most, otherwise name prefix first, also by phone digits or document. Both use `pg_trgm` GIN indexes on `name` (migration `20260929090000_search_indexes`); user text is escaped for `LIKE` (`escapeLike`).
- **Logística** (`inventory` module): `/suppliers` (name, optional RUC and WhatsApp; `409 SUPPLIER_IN_USE` when deleting one with purchases), `/purchases` (numbered per business; counted products enter stock with a `purchase` movement and their balance; `updateCosts`, default true, sets each product's cost to what was paid; `POST /purchases/:id/void` takes the stock out again with `purchase_void` movements, keeping any part already sold as `shortage`), `POST /products/:id/adjust` (reason `count`: the stock becomes what was counted; `loss` / `damage`: units leave; `correction`: signed change; always an `adjustment` movement with reason, note, balance and user — a count without difference records the check) and `GET /products?lowStock=true` (counted products with stock ≤ `minStock`, or ≤ 0 without an alert level, via a column comparison). Kardex rows carry the sale or purchase number and the adjustment reason.
- **Reports** (`/reports/*`, tabular, separate from the dashboard charts): `sales-by-customer` (walk-in sales grouped as a null customer), `sales-by-product` (revenue, and cost/profit estimated with the current cost), `shortages` (every sale line that sold a counted product beyond its stock: when, which sale, quantity, missing units and the balance left; lines of sales voided later stay listed with their status) — these three need `sales` — plus `collections-by-customer` (payments by method, always available) and `stock` (valuation at cost and at price of active counted products, negative stock counted as 0, status `out`/`low`/`ok`; needs the catalog). Ranges take `from`/`to` (inclusive, default month to date, max 3 years). Raw SQL with an explicit tenant filter (`tests/reportRepository.test.ts`). `GET /reports/:report/export?format=xlsx|pdf` renders the same rows and totals as a file in the request language (`reportExport.service.ts`): Excel via exceljs (real numbers and dates, money format, frozen header, filters, totals row) or a landscape PDF built with the shared blocks of `src/lib/pdf.ts` (brand header, tables that repeat their header on each page, footers; also used by the account statement). Sent `inline` so the app previews it before downloading.
- **Plan limits** (`src/domain/plans.ts`, `plan.service.ts`): what a business may use each month by plan and number of modules, the same numbers the landing sells. Free: 0 automatic WhatsApp messages, 1 user, 25 customers. Paid: 1 module 150 / 2 / 500, 2 modules 400 / 4 / 2,000, 3 modules 1,000 / 8 / unlimited (messages / users / customers); without Cobranza there are no automatic messages, since they are its reminders. Automatic messages are the ones Solvia sends through the provider (`Notification.automatic`); they are counted per calendar month (`APP_TIMEZONE`) and extra packs (`MessagePack`, 500 each, added by platform admins with `POST /admin/tenants/:id/message-packs`) raise the month's limit. The reminder job stops at the quota (`limited` in its summary); `POST /receivables/:id/remind` and the statement send fall back to a click-to-chat link (`whatsappUrl`, `limitReached: true`) so the owner sends it from their own WhatsApp, which is never limited. Creating a customer or a user (or reactivating one) over the limit fails with `409 PLAN_CUSTOMER_LIMIT` / `PLAN_USER_LIMIT`. `GET /settings/plan` and `GET /admin/tenants/:id/usage` return the allowance and this month's use.
- **Roadmap**: reports (best sellers, margin, stock value), accounts payable to suppliers, several warehouses, electronic receipts through a SUNAT provider.
...
| `PRODUCT_IN_USE`                                                                            | 409                          | delete a product that appears in sales                                  |
| `SALE_HAS_PAYMENTS`                                                                         | 422                          | void a credit sale whose receivable has payments                        |
| `SALE_ALREADY_VOIDED`                                                                       | 422                          | void a sale twice                                                       |
| `PURCHASE_ALREADY_VOIDED`                                                                   | 422                          | void a purchase twice                                                   |
| `SUPPLIER_IN_USE`                                                                           | 409                          | delete a supplier that has purchases                                    |
| `PRODUCT_NOT_COUNTED`                                                                       | 422                          | adjust the stock of a product that does not keep count                  |
| `PLAN_CUSTOMER_LIMIT`                                                                       | 409                          | create a customer when the plan's customer limit is reached             |
| `PLAN_USER_LIMIT`                                                                           | 409                          | create or reactivate a user when the plan's user limit is reached       |
