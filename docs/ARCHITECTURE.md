...
- **Reports** (`/reports/*`, tabular, separate from the dashboard charts): `sales-by-customer` (walk-in sales grouped as a null customer), `sales-by-product` (revenue, and cost/profit estimated with the current cost), `shortages` (every sale line that sold a counted product beyond its stock: when, which sale, quantity, missing units and the balance left; lines of sales voided later stay listed with their status) — these three need `sales` — plus `collections-by-customer` (payments by method, always available) and `stock` (valuation at cost and at price of active counted products, negative stock counted as 0, status `out`/`low`/`ok`; needs the catalog). Ranges take `from`/`to` (inclusive, default month to date, max 3 years). Raw SQL with an explicit tenant filter (`tests/reportRepository.test.ts`).
