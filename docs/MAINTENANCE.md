...
- **Heavy endpoints:**
  ...
  - `GET /dashboard/summary`, `/dashboard/cash-flow`, `/dashboard/analytics` and `/dashboard/concentration` aggregate in SQL (see ARCHITECTURE §8.5 and §8.8); the risk distribution still scores every customer, from one row per receivable. Check them with `npm run db:seed:stress` (1,000 customers, 15,000 receivables, ~20,000 payments): all under 300 ms locally.
