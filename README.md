...
| GET                   | `/dashboard/analytics`                                                                                                                                    | Period analytics: `from`, `to` (inclusive, default month to date), `granularity` (`day`                                                                                                                       | `week` | `month`, auto by default); cross-filters `method`, `customerId`, `weekday`.\|`week`\|`month`, auto by default). KPIs vs the previous period, series, payment methods, weekdays, top payers, reminders |
...
| GET/POST/PATCH/DELETE | `/products`, `/products/:id`                                                                                                                              | Product catalog (needs the `sales` or `inventory` module; `403 MODULE_NOT_ENABLED` otherwise). `search`, `status` (`active`\|`archived`\|`all`), sorting; `active: false` archives; delete is admin only      |
| GET/POST              | `/sales`, `/sales/:id`, `POST /sales/:id/void`                                                                                                            | Sales (`sales` module): cash (with `method`) or on credit (creates the receivable); counted products leave stock (may go negative, `lowStock` in the response); void puts stock back                          |
| GET                   | `/products/lookup`, `/customers/lookup`                                                                                                                   | Light picker searches (`search`, `limit` ≤ 20): exact barcode / name prefix first; customers with what they owe                                                                                               |
| GET                   | `/products/:id/movements`                                                                                                                                 | Kardex: every stock movement with `balanceAfter` and `shortage` (sold without stock)                                                                                                                          |
| GET/POST/PATCH/DELETE | `/suppliers`, `/suppliers/lookup`                                                                                                                         | Suppliers (`inventory` module)                                                                                                                                                                                |
| GET/POST              | `/purchases`, `/purchases/:id`, `POST /purchases/:id/void`                                                                                                | Purchases: counted products enter stock, costs updated; void takes it out again                                                                                                                               |
| POST                  | `/products/:id/adjust`                                                                                                                                    | Stock adjustment: count, loss, damage or correction (always traced in the kardex); `GET /products?lowStock=true` lists what is running low                                                                    |
| GET                   | `/reports/sales-by-customer`, `/reports/sales-by-product`, `/reports/shortages` (sales) · `/reports/collections-by-customer` · `/reports/stock` (catalog) | Tabular reports for a date range (`from`/`to`, default month to date): rows + totals. `/reports/:report/export?format=xlsx` or `pdf`: the same report as a file                                               |
...
| GET                   | `/settings/plan`                                                                                                                                          | Plan allowance and this month's use (automatic WhatsApp messages, users, customers)                                                                                                                           |
